...
| 路由                                                       | 方法与作用                                                                                                                                                                                                                                         |
| ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/api/agents/:id/work`                                     | GET：该同事的工作列表（`?status=` 过滤），最近更新的在前
| `/api/agents/:id/work/:workId`                             | GET：单件工作 + 步骤明细；不属于该同事时 404（不泄露别人的工作）
...
| `/api/agents/:id`、`/api/bots/:id`                         | GET/PATCH/DELETE：详情/修改/删除。两条删除路径走同一个生命周期（`AgentRuntime.removeAgent`）：忙碌时 409；删完清对话线、自己的记忆与摘要、收件箱积压、待答卡、控制条目，并从所有群的成员表移出。往来档案、任务进度、运行与工具账本作为历史事实保留。PATCH 改资料走同一个资料服务（`AgentProfileService`）：name/title/description/instructions 四个字段分开写，`avatar: null` 是明确的清空（`avatars` 接口见下一行） |
| `/api/agents/:id/avatar`                                   | GET：头像图片（只从数据目录的 `avatars/` 读，跨站来源 403）；POST：换图（body `{dataUrl}`，PNG/JPEG/WebP/GIF，<5MB）；DELETE：清空（删文件 + 字段置空）                                                                                              |
...
| `work/items.json`                                         | 工作账本（E4.1）：同事手头负责的 WorkItem 与 WorkStep，含目标、交付标准、进度、下一步、状态、revision 与产物引用。写入走原子替换；重启读回；revision 条件更新防并发覆盖                                                                                                                                                     |
| `work/waits.json`                                         | 等待账本（E4.3）：跨回合/跨重启的 WorkWait，含等谁（`correlationId`）、哪次请求、满足什么条件、到期时间与结果引用。写入走原子替换；重启读回并重建待答卡（不序列化 Promise）。密钥明文不在这里——密钥卡只存变量名，值在 `secrets.json` |
...
| `control/state.json.bak`                                 | 控制存储的上一个好版本。主文件损坏或缺失时启动会自动从它恢复，并把所有同事置为「已暂停」（需要核对，不默认放行）                                                                                                                                                   |
| `control/state.json.corrupt-<时间>`                      | 修复（`POST /api/control/repair`）时把损坏文件改名保留的现场；永不删除、不会被覆盖                                                                                                                                                                                 |
...
| 消息被拒「控制数据损坏，已进入保护模式」 | `control/state.json` 与 `state.json.bak` 都无法解析，系统进入只读保护（不写、不自动执行）。去设置 → 高级 → 修复控制数据，或调 `POST /api/control/repair` 并带 `{"confirm":"repair"}`：损坏文件会改名成 `control/state.json.corrupt-<时间>` 保留现场，然后以空状态重建，所有同事置为「已暂停」，需要逐个核对后再恢复自动处理 |
