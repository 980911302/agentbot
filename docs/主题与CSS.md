...
| 智能体气泡 | `--ai-agent-bubble` | `#eeece8` | `#252932` |
...
智能体消息：中性灰底 --ai-agent-bubble、无可见描边、圆角 --r-lg；头像 28 + 名字（身份色）+ 正文
消息正文：两边同一套 --fs-md / 行高 1.6（.msg-bubble-box），各变体只管底色、边框与内边距
输入条：底 --bg-card、边 --border、圆角 --r-xl，外缘 --scrim-bottom 渐隐托住时间线
```

智能体用中性表面气泡；群里尤其不要给每人一块彩泡，只用头像和名字颜色区分。主人右侧，它左侧；群里主人的消息靠右、不重复头像和名字。
