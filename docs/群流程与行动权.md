...
- `all_agents` 停止作用域：`POST /api/control/stop-all` 明确返回不支持；`room_round` 同样返回 `UNSUPPORTED_STOP_SCOPE`。这是 E4.4 的明确决定（不是漏做）：两者都需要持久撤销标记 + 每处 gate/effect 叠加 fence，做半套会变成「说停了、其实还有工人在跑」的假成功；理由与验收见 `docs/架构设计.md` §7.1 末段。
