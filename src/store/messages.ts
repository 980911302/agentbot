...
export class MessageStore implements MessageRepositoryPort {
  ...
  constructor(dataDir: string, options: JsonlLogOptions = {}) {
    this.log = new JsonlLog(join(dataDir, 'messages'), options);
  }
  ...
  async appendIfAbsent(message: Message): Promise<boolean> {
    ...
    if (existing) {
      if (JSON.stringify(existing.content) !== JSON.stringify(message.content))
  ...
  async latestUser(agentId: string): Promise<Message | undefined> {
    return (await this.log.view(agentId)).findLast(
      (message) =>
        message.role === 'user' &&
  ...
  async olderThan(agentId: string, newestKeep: number, coveredUpTo: number): Promise<Message[]> {
    ...
    return list
      .slice(0, Math.max(0, list.length - newestKeep))
      .filter((message) => message.createdAt > coveredUpTo);
  }

  async count(agentId: string): Promise<number> {
    return (await this.log.view(agentId)).length;
  }
  clear(agentId: string): Promise<void> {
    return this.log.clear(agentId);
  }
}
