...
/** 正文分页的偏移上限（字符）；沿用体积常量作粗上界，避免再引入一套刻度 */
const MAX_PAGE_CHARS = MAX_BYTES;
            ...
            lookup(host, { all: true })
              .then((addresses) => {
                if (!addresses.length || addresses.some((item) => isPrivateAddress(item.address)))
                  throw new UnsafeUrlError('实际连接解析到了内网/保留地址，已拒绝');
        ...
        (incoming) => {
          ...
          incoming.on('end', () => {
            if (
              ...
              reject(new Error('服务器忽略 identity 要求，返回了压缩内容；请换文本接口'));
              return;
            }
            try {
              resolve(
...
export function createWebTools(secrets: SecretStore) {
  const webFetch = defineTool<{ url: string; secretName?: string; maxChars?: number; offset?: number }>({
    ...
    description: [
      ...
      `响应体上限 ${MAX_BYTES / 1024 / 1024}MiB、单次 ${FETCH_TIMEOUT_MS / 1000} 秒、最多跟随 3 跳重定向。`,
      '需要带鉴权时，让用户通过 SendToUser(type=secret-request) 把密钥存进密钥框，再用 secretName 引用（不要索要明文）。',
    ...
    parameters: {
      ...
      properties: {
        ...
        maxChars: {
          ...
          minimum: 500,
          maximum: 12000,
          description: '本页最多返回多少字符；默认 12000，长 URL 下会按输出预算略减',
        },
        offset: {
          type: 'integer',
          minimum: 0,
          maximum: MAX_PAGE_CHARS,
          description: '纯文本**字符**偏移（不是字节），从 0 起；每页会重新抓取',
        },
      },
      required: ['url'],
    },
    async execute(args, context) {
      ...
      if (args.secretName) {
        ...
        if (!token) {
          throw new Error(
            `没有名为「${args.secretName}」的密钥；用 SendToUser(type=secret-request) 请用户填一次`,
          );
        }
      ...
      if (
        contentType &&
        !/^(text\/|application\/(json|[^;]+\+json|xml|xhtml\+xml|javascript))/i.test(contentType)
      ) {
      ...
      const limit = Math.min(
        Math.max(args.maxChars ?? MAX_TEXT_CHARS, 500),
        MAX_TEXT_CHARS,
        14000 - url.href.length - 512,
      );
      const offset = args.offset ?? 0;
      const clipped =
        text.slice(offset, offset + limit) +
        (text.length > offset + limit ? `\n[本页已满；next_offset=${offset + limit}]` : '\n[已到正文末尾]');
  ...
  const webSearch = defineTool<{ search_term: string; explanation?: string }>({
    ...
    description: [
      '用关键词搜公网，返回标题 + 链接 + 摘要。',
      '搜到结果后如果需要正文，再用 WebFetch 抓具体那一条。',
      '不要用它去访问需要登录的内容；装配了连接器工具时优先用连接器。',
    ].join(' '),
    parameters: {
      type: 'object',
      properties: {
