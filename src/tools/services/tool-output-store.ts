...
export class ToolOutputStore {
  ...
  constructor(dataDir: string, options: { maxFileBytes?: number; maxTotalBytes?: number; retentionMs?: number; maxRecords?: number } = {}) {
    ...
    if (![this.perFile, this.total, this.maxRecords].every(value => Number.isSafeInteger(value) && value > 0) || !Number.isSafeInteger(this.ttl) || this.ttl < 0) throw new Error('日志配额必须是正整数，保留期限必须是非负整数');
    ...
    for (const name of readdirSync(this.dir)) {
      ...
      if (!record.closed) {
        record.closed = true;
        if (record.execution?.state === 'running') record.execution = { ...record.execution, state: 'interrupted', exitCode: null };
        this.save(record);
      }
  ...
  create(ownerId: string, shell?: { id?: string; command: string }): OutputRecord {
    this.prune();
    while (this.records.size >= this.maxRecords && this.evictOldest()) { /* 只清理已结束日志 */ }
    ...
    const record: OutputRecord = { id, ownerId, createdAt: Date.now(), closed: false, totalBytes: 0, retainedBytes: 0, storageTruncated: false,
      ...(shell ? { command: shell.command, execution: { id, state: 'running' as const, exitCode: null } } : {}),
    };
  ...
  append(id: string, text: string): void {
    ...
    if (!record.storageTruncated) {
      while (this.bytes + Math.min(buffer.length, this.perFile - record.retainedBytes) > this.total && this.evictOldest()) { /* 不删除活跃日志 */ }
      let keep = Math.max(0, Math.min(buffer.length, this.perFile - record.retainedBytes, this.total - this.bytes));
  ...
  read(id: string, ownerId: string, offset = 0, limit = 8000): { text: string; nextOffset: number; record: OutputRecord } {
    const record = this.get(id, ownerId);
    if (!Number.isSafeInteger(offset) || offset < 0 || offset > record.retainedBytes) throw new Error('offset 超出已保存日志范围');
    if (!Number.isSafeInteger(limit) || limit < 4 || limit > 12000) throw new Error('limit 必须在 4–12000 字节之间');
    ...
    try {
      ...
      if (read && (buffer[0]! & 0xc0) === 0x80) throw new Error('offset 位于 UTF-8 字符中间，请使用返回的 next_offset');
  ...
  search(id: string, ownerId: string, query: string, offset = 0): { matches: number[]; nextOffset: number; record: OutputRecord } {
    ...
    try {
      ...
      for (let at = 0; at < count;) {
        ...
        if (matches.length >= 30) { nextOffset = offset + at; break; }
      }
      // next_offset 也可用于普通读取，避免落在多字节字符内部。
      while (nextOffset < record.retainedBytes && nextOffset - offset < count && (buffer[nextOffset - offset]! & 0xc0) === 0x80) nextOffset++;
      return { matches, nextOffset, record };
    } finally { closeSync(fd); }
  }

  private assertId(id: string): void { if (!/^[0-9a-f-]{36}$/.test(id)) throw new Error('无效的 output_id'); }
  private path(id: string): string { this.assertId(id); return join(this.dir, `${id}.log`); }
  ...
  private evictOldest(): boolean {
    const oldest = [...this.records.values()].filter(record => !this.active.has(record.id) && record.closed).sort((a, b) => a.createdAt - b.createdAt)[0];
    if (!oldest) return false;
    this.remove(oldest);
    return true;
  }
  private prune(): void {
    for (const record of this.records.values()) if (record.closed && !this.active.has(record.id) && Date.now() - record.createdAt > this.ttl) this.remove(record);
    while ((this.bytes > this.total || this.records.size > this.maxRecords) && this.evictOldest()) { /* 有界启动恢复 */ }
  }
}
