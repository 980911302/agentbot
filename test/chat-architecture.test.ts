...
const run = (id: string, status: ChatRun['status'] = 'running'): ChatRun => ({
  ...
  source: 'user',
  input: `任务${id}`,
  clientMessageId: `key-${id}`,
  status,
  createdAt: 1,
  updatedAt: 2,
});
const entry = (seq: number, payload: unknown, kind: JournalEntry['kind'] = 'run'): JournalEntry => ({
  epoch: 'e',
  seq,
  at: seq,
  agentId: 'a',
...
const snapshot = (seq: number, messages: DisplayMessage[] = [], runs: ChatRun[] = []): Snapshot => ({
  cursor: { epoch: 'e', seq },
  channels: { a: { messages, artifacts: [] } },
  runs,
});
...
describe('聊天恢复协议', () => {
  it('ready 的最新序号不跳过尚未消费的补发事件，重复事件只消费一次', async () => {
    ...
    const client = new EventClient({
      restore: async () => ({ epoch: 'e', seq: 101 }),
      apply: (e) => seen.push(e.seq),
      transport: {
        read: async (handlers, opts) => {
  ...
  it('服务重启 epoch 变化时丢弃旧游标，用新快照接收低序号事件', async () => {
    ...
    const seen: number[] = [];
    const client = new EventClient({
      restore: async () => (boot++ === 0 ? { epoch: 'old', seq: 106 } : { epoch: 'e', seq: 3 }),
      apply: (e) => seen.push(e.seq),
      transport: {
        read: async (h) => {
          h.onReady({ epoch: 'e', latestSeq: 3, resync: true && boot === 1 });
          h.onEntry(entry(4, {}));
        },
      },
    });
    ...
    assert.equal(client.cursor?.seq, 4);
  });
  ...
  it('快照读取跨过一次发布则重读，返回的水位覆盖其内容', async () => {
    ...
    assert.equal(result.cursor.seq, 1);
...
describe('无框架 ChatEngine 时序', () => {
  ...
  it('旧运行收尾不清除新运行，错误重试保存自己的原话', () => {
    ...
    assert.deepEqual(engine.respondingChannelIds, ['a']);
    ...
    engine.applyEntry(
      entry(3, {
        run: {
          ...
          error: '模型不可用',
          updatedAt: 3,
          input: long.slice(0, 500),
          inputLength: long.length,
        },
      }),
    );
  ...
  it('终态先到、回执晚到不能复活已完成的运行；finalizing 不补处理气泡', () => {
    ...
    assert.equal(engine.busy, true);
    assert.deepEqual(engine.respondingChannelIds, []);
  ...
  it('迟到历史不覆盖新回复/工具结果，新权威快照能反映清空历史', () => {
    const engine = new ChatEngine();
    engine.restore(snapshot(1, [message('old')]));
    engine.applyEntry(
      entry(
        4,
        {
  ...
  it('另一个页面发送的用户消息会进入时间线，重放不会重复', () => {
    ...
    const e = {
      ...
      message: {
        ...
        agentId: 'a',
        role: 'user',
        content: { type: 'text', text: '别处发的' },
        createdAt: 4,
      },
    };
    engine.applyEntry(entry(1, e, 'agent'));
    engine.applyEntry(entry(2, e, 'agent'));
    assert.equal(engine.histories.a?.length, 1);
    assert.equal(engine.histories.a?.[0]?.role, 'user');
  });
  ...
  it('未知受理结果的重试保留原幂等键，确认受理后清掉网络错误', () => {
    const engine = new ChatEngine();
    engine.beginSend('a', 'key-A', '任务A', '我');
  ...
  it('只有对应运行的未确认工具卡被收口', () => {
    ...
    const tool = (id: string) => ({
      ...message(id),
      runId: id,
      toolCalls: [{ id: `${id}-call`, name: 'Read', arguments: '{}', status: 'running' as const }],
    });
...
describe('真实 Runtime 统一运行入口（假模型）', () => {
  it('群内显式私发按持久消息目的地路由，不误投群频道', async () => {
    ...
    try {
      ...
      const { run: r } = await coordinator.prepare({
        channelId: 'room',
        roomId: 'room',
        agentId: 'a',
        kind: 'agent',
      ...
      coordinator.bind(r).onEvent?.({
        type: 'message',
        message: {
          id: 'private',
          agentId: 'a',
          role: 'assistant',
          content: { type: 'text', text: '私发' },
          createdAt: 1,
        },
      });
      const event = journal.since(0).entries.find((e) => e.kind === 'agent')!;
      assert.equal(event.agentId, 'a');
      assert.equal(event.roomId, undefined);
      assert.equal(event.runId, r.runId);
    } finally {
      await tmp.cleanup();
    }
  });
  ...
  it('并发重复发送 + 并发 execute 只产生一条用户消息和一次模型调用', async () => {
    ...
    try {
      ...
      const runtime = runtimeAt(tmp.dir, provider);
      const agent = await runtime.registry.create({ name: '测试', instructions: '测试' });
      const accepted = await Promise.all(
        Array.from({ length: 8 }, () => runtime.acceptMessage(agent.id, '你好', { clientMessageId: 'same' })),
      );
      ...
      assert.equal(
        runtime.events.since(0).entries.filter((e) => e.runId === id && (e.payload as any).phase === 'done')
          .length,
        1,
      );
      await assert.rejects(
        runtime.acceptMessage(agent.id, '不同内容', { clientMessageId: 'same' }),
        /不同请求/,
      );
    } finally {
      await tmp.cleanup();
  ...
  it('自动续跑走相同事件出口，三次执行独立身份并关联原任务', async () => {
    ...
    try {
      ...
      const runtime = runtimeAt(tmp.dir, provider);
      const agent = await runtime.registry.create({ name: '测试', instructions: '测试' });
      const a = await runtime.acceptMessage(agent.id, '任务A');
      const first = a.execute();
      await waitFor(() => provider.calls.length === 1, 'A 开始');
      const b = await runtime.acceptMessage(agent.id, '任务B');
      const second = b.execute();
      await waitFor(() => provider.calls.length === 2, 'B 开始');
      provider.releaseText(1, 'B完成');
      await second;
      await first;
      await waitFor(() => provider.calls.length === 3, 'A 自动续跑');
      provider.releaseText(2, 'A续跑完成');
      await waitFor(
        () => runtime.chatRuns.list().filter((r) => r.status === 'succeeded').length === 2,
        '续跑终态',
      );
      ...
      await waitFor(
        () =>
          runtime.events
            .since(0)
            .entries.filter((e) => e.kind === 'run' && (e.payload as any).phase === 'done').length === 3,
        '三条 done 事件',
      );
      const resumed = runtime.chatRuns.list().find((r) => r.source === 'resume')!;
      assert.ok(resumed);
      assert.equal(resumed.parentRunId, a.receipt.runId);
      assert.equal(resumed.taskId, a.receipt.taskId);
      const events = runtime.events.since(0).entries;
      assert.ok(
        events.some(
          (e) => e.runId === resumed.runId && (e.payload as any).message?.content?.text === 'A续跑完成',
        ),
      );
  ...
  it('群入站先落盘入队，成员独立运行，相同命令不会重复广播', async () => {
    ...
    try {
      const provider = new FakeProvider({ auto: () => FakeProvider.text('') });
      const runtime = runtimeAt(tmp.dir, provider);
      const agent = await runtime.registry.create({ name: '测试', instructions: '测试' });
      const room = await runtime.rooms.create({ name: '群', memberIds: [agent.id] });
      const [a, b] = await Promise.all([
        runtime.acceptRoomMessage(room.id, '你好', { clientMessageId: 'room-key' }),
        runtime.acceptRoomMessage(room.id, '你好', { clientMessageId: 'room-key' }),
      ]);
      ...
      assert.notEqual(child.parentRunId, a.receipt.runId);
      assert.notEqual(child.taskId, a.receipt.taskId);
  ...
  it('私聊空回复明确失败，不能当作已完成；群聊空回复仍允许沉默', async () => {
    ...
    try {
      ...
      assert.equal(state.channels[agent.id]?.messages.length, 1);
    } finally {
      await tmp.cleanup();
    }
  });
  ...
  it('进程重建把未完成运行标记 interrupted，不自动重复工具；幂等键仍有效', async () => {
    ...
    try {
      ...
      const accepted = await first.prepare({
        channelId: 'a',
        agentId: 'a',
        kind: 'agent',
        source: 'user',
      ...
      const duplicate = await second.prepare({
        channelId: 'a',
        agentId: 'a',
        kind: 'agent',
        source: 'user',
  ...
  it('事件观察者抛错不改变执行结果', async () => {
    ...
    try {
      ...
      const options = coordinator.bind(r, {
        onEvent: () => {
      ...
      await coordinator.execute(
        r.runId,
        async () => {
          options.onEvent?.({ type: 'final', content: '完成' });
          return {};
        },
        () => ({}),
      );
