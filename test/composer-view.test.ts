...
describe('composerAreaSize：输入框 1~8 行自适应（UI-07）', () => {
  ...
  it('再长也还是封在 8 行', () => {
    assert.deepEqual(composerAreaSize(100 * COMPOSER_LINE_HEIGHT + 8), { height: COMPOSER_MAX_HEIGHT, scrolls: true });
  });
  ...
  it('空内容与非法值回落到最小高度', () => {
    ...
    assert.deepEqual(composerAreaSize(Number.POSITIVE_INFINITY), { height: COMPOSER_MIN_HEIGHT, scrolls: false });
  });
...
describe('composerPlaceholder：占位文案', () => {
  it('忙碌时说明发送会插话（优先于群聊提示）', () => {
    assert.equal(composerPlaceholder({ busy: true, isGroup: false, botName: '小白' }), '它正在工作，发送会插话');
    assert.equal(composerPlaceholder({ busy: true, isGroup: true, botName: '小白' }), '它正在工作，发送会插话');
  });

  it('群里提示 @ 唤醒成员', () => {
    assert.equal(composerPlaceholder({ busy: false, isGroup: true, botName: '' }), '在群聊中发消息，输入 @ 唤醒指定成员…');
  });
