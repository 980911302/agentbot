...
const transfer = (id: string, from = a, to = b): Correspondence => ({ id, from, to, text: `消息${id}`, createdAt: Number(id) || 1 });
const input = (patch: Partial<Message> = {}): Message => ({ id: 'msg', agentId: 'b', role: 'user', content: { type: 'text', text: '你好' }, createdAt: 1, ...patch });
const runtimeAt = (dir: string) => new AgentRuntime({ dataDir: dir, tools: [], createProvider: () => new FakeProvider({ auto: () => FakeProvider.text('收到') }),
...
describe('模型角色与真实消息身份分离', () => {
  it('真实用户才显示右侧气泡；旧的同事来信保留来源而非冒充用户', () => {
    ...
    assert.equal(old.role, 'assistant'); assert.equal(old.senderName, '架构师'); assert.equal(old.originLabel, '消息来自 架构师');
    const unknown = messageIdentity(input({ source: 'agent' }));
    assert.equal(unknown.role, 'assistant'); assert.match(unknown.originLabel!, /来源未记录/);
  });
  it('群里的用户发言也必须标群来源，不伪装成当前私聊输入', () => {
    const identity = messageIdentity(input({ source: 'room', roomId: 'r', roomName: '开发组', speaker: '主人' }));
    assert.equal(identity.role, 'assistant'); assert.match(identity.originLabel!, /开发组.*主人/);
  });
  it('新消息按稳定发送者身份，旧群 assistant 不继承入站 speaker 冒充同事', () => {
    assert.equal(messageIdentity(input({ source: 'agent', sender: a, speaker: '错误的旧名字' })).senderName, a.name);
    const old = messageIdentity(input({ role: 'assistant', source: 'room', speaker: '错误的入站作者', roomName: '群' }));
    assert.equal(old.senderName, undefined); assert.equal(old.originLabel, '发言于「群」');
  });
  it('同事/群来信与待发文本相同，也不能替换本地用户占位', () => {
    const pending = { id: 'pending-key', role: 'user' as const, content: '你好', toolCalls: [], createdAt: new Date(0).toISOString() };
    for (const source of ['agent', 'room'] as const) {
      const next = applyEvent([pending], { type: 'message', message: input({ source, speaker: a.name }) });
      assert.equal(next.length, 2); assert.equal(next[0]!.id, 'pending-key'); assert.equal(next[1]!.role, 'assistant');
    }
  });
  ...
  it('模型历史和摘要保留作者/群来源，而不是只修气泡颜色', () => {
    ...
    assert.equal(llm[0]!.role, 'user'); assert.match(llm[0]!.content!, /消息来自 幕僚/);
  ...
  it('合批输入不重复显示；往来事件可重放且计数基于投递 id', () => {
    ...
    assert.equal(toDisplayMessages([task]).length, 0); assert.equal(applyEvent([], { type: 'message', message: task }).length, 0);
    const event = { type: 'correspondence' as const, transfer: transfer('1') };
    const shown = applyEvent(applyEvent([], event), event); assert.equal(shown.length, 1);
    const rows = chatRows([...shown, toCorrespondenceView(transfer('2', b, a)), ...shown]);
    assert.equal(rows.length, 1); assert.equal(rows[0]!.kind, 'correspondence');
    if (rows[0]!.kind !== 'correspondence') return;
    assert.equal(rows[0]!.transfers.length, 2); assert.equal(transferLabel('a', rows[0]!.transfers), '2 条消息往来');
    assert.deepEqual(transferPeers('a', rows[0]!.transfers).map(peer => peer.id), ['b']);
...
describe('可追溯的智能体往来档案', () => {
  it('同一投递幂等归档，重启与分页只返回双方消息', async () => {
    ...
    try {
      ...
      assert.deepEqual(page.messages.map(m => m.id), ['3', '4']); assert.equal(page.nextBefore, '3');
      assert.deepEqual((await reopened.page('a', 'b', page.nextBefore!, 2)).messages.map(m => m.id), ['1', '2']);
  ...
  it('SendToAgent 真实投递双向可见，消费后不丢档案，改名不改历史身份', async () => {
    const tmp = await tempDataDir('correspondence-runtime'); const runtime = runtimeAt(tmp.dir);
    try {
      const sender = await runtime.registry.create({ name: a.name }); const receiver = await runtime.registry.create({ name: b.name });
      ...
      const entries = await runtime.correspondence.list(sender.id); assert.equal(entries.length, 1);
      assert.equal(entries[0]!.from.id, sender.id); assert.equal(entries[0]!.to.id, receiver.id);
      ...
      const engine = new ChatEngine(); for (const entry of before) engine.applyEntry(entry);
      assert.equal(engine.histories[sender.id]!.filter(m => m.correspondence).length, 2);
      const snapshot = await runtime.chatSnapshot([sender.id, receiver.id]); engine.restore(snapshot);
      assert.equal(engine.histories[sender.id]!.filter(m => m.correspondence).length, 2);
  ...
  it('多个同事逐封处理时保留每封原始信和各自发送者', async () => {
    const tmp = await tempDataDir('correspondence-batch'); const runtime = runtimeAt(tmp.dir);
    try {
      const receiver = await runtime.registry.create({ name: '收件人' });
      for (const actor of [a, b]) await runtime.inbox.enqueue({ toAgentId: receiver.id, fromAgentId: actor.id, fromName: actor.name,
        fromActor: actor, text: `${actor.name}的结果`, priority: false, depth: 1 });
      await runtime.drainInbox(receiver.id);
      await runtime.drainInbox(receiver.id);
      const snapshot = await runtime.displayMessages(receiver.id);
      assert.equal(snapshot.filter(m => m.correspondence).length, 2);
      assert.deepEqual(snapshot.flatMap(m => m.correspondence ? [m.correspondence.from.id] : []), ['a', 'b']);
      assert.ok(!snapshot.some(m => m.role === 'user'));
    } finally { await runtime.close(); await tmp.cleanup(); }
  });
  it('入队后未归档就中断，启动恢复补档且不重复', async () => {
    const tmp = await tempDataDir('correspondence-recover'); const runtime = runtimeAt(tmp.dir);
    let reopened: AgentRuntime | undefined;
    try {
      const receiver = await runtime.registry.create({ name: '收件人' });
      const item = await runtime.inbox.enqueue({ toAgentId: receiver.id, fromAgentId: 'a', fromName: a.name, fromActor: a,
        text: '已受理但未归档', priority: false, depth: 1 });
      await runtime.close(); reopened = runtimeAt(tmp.dir); await reopened.recover();
      assert.equal((await reopened.correspondence.list(receiver.id)).filter(m => m.id === item.id).length, 1);
    } finally { await reopened?.close(); await runtime.close(); await tmp.cleanup(); }
  });
  it('群代发的真实作者贯穿群时间线、成员经历与排队恢复', async () => {
    const tmp = await tempDataDir('correspondence-room'); const runtime = runtimeAt(tmp.dir);
    try {
      const sender = await runtime.registry.create({ name: '幕僚' }); const receiver = await runtime.registry.create({ name: '测试' });
      ...
      const dispatcher = new RoomDispatcher({ registry: runtime.registry, rooms: runtime.rooms, messages: runtime.messages, inbox: runtime.inbox,
        ...
        runTurn: async (_id, task) => { tasks.push(task); return { content: '' }; } });
      await dispatcher.postToRoom(room.id, '请接收延迟消息', { roomSenderId: sender.id, excludeAgentIds: [sender.id] });
      const item = (await runtime.inbox.peek(receiver.id))[0]!; assert.equal(item.fromActor?.id, sender.id);
      locks.clear(); await dispatcher.deliverQueued(item);
      assert.equal(tasks[0]!.sender?.id, sender.id); assert.equal(tasks[0]!.sender?.kind, 'agent');
    } finally { await runtime.close(); await tmp.cleanup(); }
  });
  it('往来详情 API 只读、限制分页，不暴露第三方私聊', async () => {
    const tmp = await tempDataDir('correspondence-http');
    const server = await createAgentServer({ port: 0, rootDir: tmp.dir, dataDir: tmp.dir, allowMissingKey: true,
    ...
    try {
      ...
      assert.deepEqual((await response.json()).messages.map((m: Correspondence) => m.id), ['1']);
      assert.equal((await fetch(`${url}?limit=500`)).status, 400);
      assert.equal((await fetch(url, { method: 'POST' })).status, 404);
    } finally { await server.close(); await tmp.cleanup(); }
  });
});
