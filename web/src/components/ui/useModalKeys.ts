...
import { isTabbable, isTopmostDialog, nextFocusIndex, popDialog, pushDialog } from '../../a11y.js';
...
export function useModalKeys({ open, onClose, id, trapFocus = true }: UseModalKeysOptions) {
  ...
  useEffect(() => {
    ...
    // 打开即聚焦首个可聚焦元素；没有可聚焦控件时退到容器本身
    const initial = tabbable()[0] ?? container;
