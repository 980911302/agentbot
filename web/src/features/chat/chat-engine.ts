import {
  ...
  type ChatSnapshot,
  type JournalEntry,
} from '../../../../src/shared/contracts/chat-state';
...
export class ChatEngine {
  ...
  setInteractions = (list: InteractionRequest[]) => {
  ...
  setHistories = (
    update:
      ...
      | ((prev: Record<string, DisplayMessage[]>) => Record<string, DisplayMessage[]>),
  ) => {
  ...
  sendFailed(key: string, reason: string): void {
    ...
    if (
      ...
      [...this.runs.values()].some(
        (run) => run.clientMessageId === key && run.channelId === pending.channelId,
      )
    )
      return;
    ...
    this.histories = {
      ...this.histories,
      [pending.channelId]: [
        ...(this.histories[pending.channelId] ?? []).filter((m) => m.id !== error.id),
        error,
      ],
    };
    this.changed();
  ...
  restore(snapshot: Snapshot): void {
    ...
    if (snapshot.interactions && snapshot.cursor.seq >= this.interactionSeq) {
      ...
      this.interactionSeq = snapshot.cursor.seq;
    }
        ...
        ? current.filter(
            (message) =>
              ...
              (message.clientMessageId &&
                [...this.runs.values()].some(
                  (run) => run.clientMessageId === message.clientMessageId && isActiveChatRun(run),
                )),
          )
        : current;
  ...
  applyEntry(entry: JournalEntry): boolean {
    ...
    if (entry.kind === 'agent' && entry.seq > this.interactionSeq) {
      ...
      if (event.type === 'interaction') {
        this.interactions = [
          ...this.interactions.filter((item) => item.id !== event.request.id),
          event.request,
        ];
    ...
    if (entry.kind === 'run') {
      ...
      if (entry.kind === 'agent' && !entry.roomId) {
        ...
        if (event.type === 'message' || event.type === 'correspondence') {
          this.histories = {
            ...this.histories,
            [channelId]: applyEvent(this.histories[channelId] ?? [], event),
          };
          if (
            event.type === 'message' &&
        ...
        if (event.type === 'room_message') {
          ...
          this.histories = {
            ...
            [channelId]: [
              ...list.filter(
                (item) =>
                  item.id !== m.id && !(m.clientMessageId && item.id === `pending-${m.clientMessageId}`),
              ),
              message,
            ],
          };
  ...
  private applyRun(run: ChatRun, seq: number): void {
                ...
                toolCalls: message.toolCalls.map((call) =>
                  call.status === 'running'
                    ? {
                        ...
                        result: '本次执行已结束，工具结果尚未确认；请核对后再继续。',
                      }
                    : call,
                ),
              }
    ...
    if (run.clientMessageId) {
      ...
      this.histories = {
        ...
        [run.channelId]: list
          ...
          .map((m) =>
            m.id === `pending-${key}` && run.messageId ? { ...m, id: run.messageId, runId: run.runId } : m,
          ),
      };
    }
    ...
    const finished = [...this.runs.values()].filter(
      (item) => !isActiveChatRun(item) && item.status !== 'parked',
    );
    for (const old of finished.slice(0, Math.max(0, finished.length - 1000))) {
      this.runs.delete(old.runId);
      this.runSeq.delete(old.runId);
      this.live.delete(old.runId);
    }
  }

  get busy(): boolean {
    return (
      [...this.runs.values()].some(isActiveChatRun) || [...this.pending.values()].some((p) => !p.uncertain)
    );
  }
  get respondingChannelIds(): string[] {
    return [
      ...new Set([
        ...[...this.runs.values()]
          .filter((run) => run.status === 'queued' || run.status === 'running')
          .map((run) => run.channelId),
  ...
  liveFor(channelId: string): string {
    return [...this.runs.values()]
      .filter((run) => run.channelId === channelId && isActiveChatRun(run))
      .map((run) => this.live.get(run.runId) ?? '')
      .join('\n')
      .trim();
  }
}
